...
exports.getRecordsByUser = async (req, res) => {
  try {
    ...
    if (!records.length) return res.status(404).json({ message: "No service records found" });
...
exports.updateRecord = async (req, res) => {
  try {
    ...
    if (!ServiceRecordId) return res.status(400).json({ message: "ServiceRecordId is required" });

    const updatedRecord = await ServiceRecordModel.findByIdAndUpdate(ServiceRecordId, updates, { new: true });
